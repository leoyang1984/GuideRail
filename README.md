...
1. Download the latest `GuideRail-0.6.9-chrome-*.zip` from GitHub Releases and extract it to a permanent local directory (e.g. your documents or tools folder);
...
1. Download `GuideRail-0.6.9-obsidian-companion-*.zip` from GitHub Releases;
2. Open your Obsidian Vault directory and navigate to `.obsidian/plugins/` (create the `plugins` folder if it does not exist);
     ...
     .obsidian/
       plugins/
         obsidian-companion/
           ...
           main.js
   ```
4. In Obsidian, go to **Settings -> Community plugins**, reload the plugin list, find **GuideRail Companion**, and enable it.
...
> [!NOTE]
> If you do not use Obsidian, you can also click "Select Local Folder" in the sidepanel to directly grant permissions to any local folder on your disk.

#### Step 4: One-Click Clipping in ChatGPT
...
3. The button instantly changes to **"✓ 已收藏" (Bookmarked)**:
   - The response Markdown content and any generated image attachments are saved into your Vault's `收件箱/` directory;
   - The sidepanel automatically updates and displays the new note in real-time.
...
- `npm test`: Runs all 237 automated unit and integration tests.
- `npm run build`: Type-checks and builds the Chrome extension into `dist/`.
- `npm run build:companion`: Builds the Obsidian Companion plugin into `packages/obsidian-plugin/main.js`.
- `npm run release:local`: Runs privacy check, tests, and packages release zips.
...
### 第一步：安装 Chrome 扩展
1. 前往 GitHub Releases 下载最新的 `GuideRail-0.6.9-chrome-*.zip` 并解压到本地固定目录（例如你的文档或工具文件夹）；
...
1. 在 Releases 中下载 `GuideRail-0.6.9-obsidian-companion-*.zip`；
2. 打开你的 Obsidian 笔记库（Vault）所在目录，进入 `.obsidian/plugins/`（如没有 `plugins` 文件夹可新建一个）；
3. 在 `plugins` 下新建文件夹 `obsidian-companion`，将压缩包内的 `main.js`、`manifest.json` 解压至该目录：
   ```text
   <你的 Vault 根目录>/
     .obsidian/
       plugins/
         obsidian-companion/
           ...
           main.js
   ```
4. 打开 Obsidian，进入 **设置 -> 第三方插件 (Community plugins)**，刷新插件列表后找到 **GuideRail Companion** 并开启右侧开关。

### 第三步：两端配对与连接
1. 在 Obsidian 中进入 **GuideRail Companion** 设置页面：
   - **保存根目录**：建议**保持空白（留空）**。留空代表直接保存在 Vault 根目录的“收件箱/”文件夹中；
   - 点击 **【生成配对码】**，8 位配对码会**自动复制到系统剪贴板**，右侧也提供【复制配对码】按钮；
2. 点击 Chrome 工具栏上的 GuideRail 图标打开侧边栏（Sidepanel）；
3. 在侧边栏的配对界面中，粘贴刚才生成的 8 位配对码，点击 **【连接 Obsidian】**；
4. 侧边栏将立即显示 `Obsidian Companion (已连接: <你的 Vault 名称>)`，配对即刻完成！

> [!NOTE]
> 如果不使用 Obsidian，也可以在扩展侧边栏中点击“选择保存文件夹”，直接授权本地普通磁盘目录进行保存。

### 第四步：在 ChatGPT 中一键收藏
1. 打开任意 `https://chatgpt.com/c/<对话ID>` 页面（初次安装或更新扩展后，请**刷新一次**已打开的 ChatGPT 页面）；
2. 浏览对话，在任意助手回复的下方操作栏中，会看到 GuideRail 注入的 **「☆ 收藏」** 按钮；
3. 点击「☆ 收藏」，按钮即刻变为 **「✓ 已收藏」**：
   - 回复正文（Markdown 格式）与图片附件会自动写入 Obsidian Vault 的 `收件箱/` 目录；
   - 侧边栏会自动同步展示这条新笔记。
...
- `npm test`：运行全部自动化单元测试（涵盖鉴权、协议、写入、去重、防抖等 237 项测试）。
- `npm run build`：执行 TypeScript 检查并打包 Chrome 扩展至 `dist/`。
- `npm run build:companion`：打包 Obsidian Companion 插件至 `packages/obsidian-plugin/main.js`。
- `npm run release:local`：执行完整隐私扫描、测试和发布打包，生成源码包、Chrome 扩展包及 Obsidian 插件包。

---

## 许可证

本项目采用 [MIT 许可证](LICENSE)。欢迎提交 Issue 或 Pull Request！
