...
export function applyCommand(source: GuideRailStorage, command: Command, now = new Date().toISOString(), id: () => string = () => crypto.randomUUID()): GuideRailStorage {
  ...
  switch (command.type) {
    ...
    case 'saveReply': {
      ...
      if (saved) break;
      const sourceId = id(); saved = { ...capture, id: sourceId, createdAt: now, updatedAt: now };
      if (command.preferredTitle) saved.title = command.preferredTitle;
      data.sources[sourceId] = saved;
