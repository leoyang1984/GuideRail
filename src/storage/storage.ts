...
export function createDispatcher(repository: StorageRepository) {
  ...
  return (command: Command): Promise<void> => {
    const operation = tail.then(async () => {
      const commit = async () => { const next = applyCommand(await repository.read(), command); await repository.write(validateBackup(next)); };
      if (typeof navigator !== 'undefined' && navigator.locks) await navigator.locks.request('guiderail-legacy', commit);
      else await commit();
    });
