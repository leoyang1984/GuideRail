...
export function ReplyBody({ html, text, title, localImages = {} }: { html?: string; text: string; title?: string; localImages?: Record<string, string> }) {
  ...
  function render(node: Node, key: number, depth = 0): ReactNode {
    ...
    if (tag === 'img') { const path = node.getAttribute('src') ?? ''; const src = Object.hasOwn(localImages, path) ? localImages[path] : undefined; return src ? createElement('img', { key, src, alt: node.getAttribute('alt') || '收藏图片', style: { maxWidth: '100%', height: 'auto' } }) : Object.keys(localImages).length ? createElement('span', { key }, '[图片未保存在本地]') : null; }
