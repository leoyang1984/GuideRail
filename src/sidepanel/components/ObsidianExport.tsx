...
import { ensureDirectoryPermission, loadDirectory, storeDirectory, writeNewFile, type DirectoryPicker, type ExportDirectory } from '../../export/directory';
...
export function ObsidianExport({ sources, label }: { sources: MessageSource[]; label: string }) {
  ...
  async function exportReplies() {
    ...
    try {
      // Only prompt from this user gesture when the remembered permission is no longer granted.
      if (!await ensureDirectoryPermission(directory)) {
