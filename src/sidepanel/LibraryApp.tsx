...
export function LibraryApp() {
  ...
  return <main className="reader-app" data-theme={theme !== 'system' ? theme : undefined}><header className="app-header"><h1><span className="brand-mark">G</span>GuideRail</h1>{!selected && <div className="app-header-right"><span className="app-tagline">{t('tagline')}</span>{ready && <button type="button" className="theme-quick-btn" title={t('themeButtonTitle')} onClick={cycleTheme}>🎨 {THEMES.find(th => th.id === theme) ? t(THEMES.find(th => th.id === theme)!.nameKey) : t('themeButtonLabel')}</button>}</div>}</header>
    {loading ? <p>{t('loadingLibrary')}</p> : <>
      {!ready ? backendMode === 'companion' ? <CompanionSetup busy={busy} error={backendError} action={action} onConnected={async name => { await refresh(); setNotice(t('companionConnectedNotice', { name })); }} onUseDirectory={useDirectory} /> : listError ? <div role="alert" className="library-feedback error">{listError}<button disabled={busy} onClick={() => void refresh()}>{t('retry')}</button></div> : <section className="library-setup">
        ...
        {!selected && <>
        <details className="data-settings bear-drawer"><summary className="bear-settings-btn"><span className="settings-summary-left"><span className="settings-icon" aria-hidden="true"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12.22 2h-.44a2 2 0 0 0-2 2v.18a2 2 0 0 1-1 1.73l-.43.25a2 2 0 0 1-2 0l-.15-.08a2 2 0 0 0-2.73.73l-.22.38a2 2 0 0 0 .73 2.73l.15.1a2 2 0 0 1 1 1.72v.51a2 2 0 0 1-1 1.74l-.15.09a2 2 0 0 0-.73 2.73l.22.38a2 2 0 0 0 2.73.73l.15-.08a2 2 0 0 1 2 0l.43.25a2 2 0 0 1 1 1.73V20a2 2 0 0 0 2 2h.44a2 2 0 0 0 2-2v-.18a2 2 0 0 1 1-1.73l.43-.25a2 2 0 0 1 2 0l.15.08a2 2 0 0 0 2.73-.73l.22-.39a2 2 0 0 0-.73-2.73l-.15-.08a2 2 0 0 1-1-1.74v-.5a2 2 0 0 1 1-1.74l.15-.09a2 2 0 0 0 .73-2.73l-.22-.38a2 2 0 0 0-2.73-.73l-.15.08a2 2 0 0 1-2 0l-.43-.25a2 2 0 0 1-1-1.73V4a2 2 0 0 0-2-2z"/><circle cx="12" cy="12" r="3"/></svg></span><span className="settings-title">{t('settings')}</span></span><span className="settings-summary-right"><span className="settings-summary-value">{root?.name}</span><span className="settings-chevron" aria-hidden="true"><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="6 9 12 15 18 9" /></svg></span></span></summary>
