...
try {
  ...
  const companionDirectory = resolve('packages/obsidian-plugin');
  for (const name of ['main.js', 'manifest.json', 'README.md']) check(`${companionDirectory}/${name}`);
  const companionZip = resolve(`release/GuideRail-${version}-obsidian-companion-${stamp}.zip`);
  execFileSync('zip', ['-X', '-q', companionZip, 'main.js', 'manifest.json', 'README.md'], { cwd: companionDirectory });
  console.log(`Source (no Git history): ${sourceZip}\nChrome extension: ${extensionZip}\nObsidian Companion: ${companionZip}`);
} finally {
