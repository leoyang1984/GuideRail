...
import { ensureDirectoryPermission, loadDirectory, storeDirectory, writeNewFile } from '../src/export/directory';
vi.mock('../src/export/directory', () => ({ ensureDirectoryPermission: vi.fn(async () => true), loadDirectory: vi.fn(async () => null), storeDirectory: vi.fn(async () => {}), writeNewFile: vi.fn(async () => 'written') }));
...
it('selects an export directory and exports the full original heading', async () => {
  ...
  await click('导出这条回复');
  expect(ensureDirectoryPermission).toHaveBeenCalledWith(directory);
