...
it.each([320, 360, 390, 720, 1024].flatMap(width => [false, true].map(dark => ({ width, dark }))))('supports $width px / dark=$dark without fixed-width content', ({ width, dark }) => {
  ...
  expect(style('button.primary').color).toBe(dark ? 'rgb(32, 33, 36)' : 'rgb(255, 255, 255)');
  expect(style('main').color).toBe(dark ? 'rgb(232, 234, 237)' : 'rgb(44, 42, 41)');
...
it('ensures settings drawer has zero padding and centered summary line', () => {
  const { style } = resolved(390, false);
  document.querySelector('main')!.insertAdjacentHTML('beforeend', '<details class="data-settings bear-drawer"><summary class="bear-settings-btn"><span class="settings-summary-left">设置</span></summary></details>');
  expect(style('.data-settings').paddingTop).toBe('0px');
  expect(style('.data-settings').paddingBottom).toBe('0px');
  expect(style('.data-settings > summary').height).toBe('38px');
  expect(style('.data-settings > summary').display).toBe('flex');
  expect(style('.data-settings > summary').alignItems).toBe('center');
});

it('ensures reading-tools buttons have unified 32px height and border alignment', () => {
  const { style } = resolved(390, false);
  document.querySelector('main')!.insertAdjacentHTML('beforeend', '<div class="reading-tools"><button type="button" class="back-button">← 返回</button><button type="button" class="reading-origin-btn">回到原回复</button><details class="reply-more library-actions"><summary aria-label="更多操作"><svg width="16" height="16"></svg></summary></details></div>');
  expect(style('.back-button').height).toBe('32px');
  expect(style('.reading-origin-btn').height).toBe('32px');
  expect(style('.library-actions > summary').height).toBe('32px');
  expect(style('.library-actions > summary').width).toBe('32px');
  expect(style('.back-button').borderWidth).toBe('1px');
  expect(style('.reading-origin-btn').borderWidth).toBe('1px');
  expect(style('.library-actions > summary').borderWidth).toBe('1px');
});
